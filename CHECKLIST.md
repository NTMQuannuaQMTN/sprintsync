...
- **Update (later pass):** A real commits page now exists (`apps/web/src/app/repositories/[id]/commits/page.tsx`, nav entry in `Sidebar.tsx`) with expandable per-file diffs, plus a manual "Update to tasks" button — see WEBHOOK-004 below.

### BUG-019 — Commits page showed nothing for any repo whose history predates its webhook, because nothing ever called GitHub's list-commits API
- **Status:** FIXED
- **Evidence:** `GitHubService.get_commits()` (`apps/api/src/services/github.py:71`) was fully implemented but grepped to zero call sites anywhere in `src/` — the only path that ever wrote a `Commit` row was `webhook.py`, triggered per real-time GitHub push event. This means a repo's entire pre-connection commit history, or any repo whose webhook wasn't actively firing, showed an empty "No commits yet" state forever — confirmed live: the real `NTMQuannuaQMTN/sprintsync` repo connected this session had 0 stored `Commit` rows despite dozens of real commits on GitHub.
- **Fix:** `GET /repositories/{repo_id}/commits` (`apps/api/src/api/v1/commits.py`) now calls a new `_sync_commits_from_github()` before reading from the DB — fetches the latest commits from GitHub via the repo owner's stored token, dedupes against already-stored SHAs, and inserts the new ones (concurrently enriched with per-file diff/stat data via `asyncio.gather` over `get_commit_detail`, same as `webhook.py`'s enrichment). Best-effort: no token, unreachable GitHub, or a renamed branch just falls through to the existing local rows.
- **Live verification:** ran `_sync_commits_from_github` directly against the real repo/real token — went from 0 stored commits to 10, matching GitHub's actual history (correct messages, authors, diffs), then cleaned up the test-inserted rows so the real app performs the same sync fresh on next page load.
- **Test:** `test_list_commits_still_returns_locally_stored_commits_without_a_github_token` — guards the common case (no token yet) so the new sync call can't break the existing local-only listing.

### WEBHOOK-004 — Manual "Update to tasks": re-run AI analysis over already-stored commits on demand
- **Status:** COMPLETED
- **Evidence:** New `POST /repositories/{repo_id}/commits/analyze` (`apps/api/src/api/v1/commits.py`) reuses the same `ai_service.analyze_commit` heuristic and open-tasks query the webhook already uses per-commit, but runs it in bulk over every stored `Commit` row with `analyzed = False`, creates pending `Suggestion` rows exactly like the webhook path (same approve/reject review flow — this endpoint never mutates a task's status directly), then marks each processed commit `analyzed = True` so re-clicking the button doesn't duplicate suggestions. `webhook.py` was also updated to set `analyzed = True` right after it runs analysis on a freshly-ingested commit, for the same reason. Frontend: "Update to tasks" button on the commits page (`apps/web/src/app/repositories/[id]/commits/page.tsx`, via `AppShell`'s `headerActions`), with an inline result banner ("Reviewed N commits and created M suggestions — Review suggestions") and error state; `commitsApi.analyze` in `apps/web/src/lib/api.ts`.
- **Test:** `apps/api/tests/test_repository_flow_integration.py::test_update_to_tasks_button_analyzes_unanalyzed_commits` — inserts a real, never-analyzed commit row directly (simulating one that predates this feature), calls the endpoint, asserts a real suggestion is created and the task's status is untouched until approved, and that a second call processes 0 commits (no duplicate suggestions).
- **Also fixed while here (BUG-018):** `webhook.py` created every `Suggestion` without setting `commit_id`, so `suggestions.py`'s `_enrich()` (which joins through `commit_id` to fill `commit_sha`/`commit_message`) silently returned `null` for those fields on every webhook-created suggestion — a real, previously-undetected gap, since no existing test asserted on `commit_sha`. Fixed by setting `commit_id=commit.id` when constructing the `Suggestion`, which in turn required adding an explicit `await db.flush()` after `db.add(commit)` (this app's sessions run with `autoflush=False`, and `Commit.id` is a client-side `uuid.uuid4()` default that SQLAlchemy only populates on flush — without the flush, `commit.id` would still read `None` at that point). Caught by adding `assert suggestion["commit_sha"] is not None` to the existing webhook integration test, not by manual inspection.
...
### BUG-016 — Spec-upload task extraction silently produced zero real tasks on any non-English, non-bulleted document
- **Status:** FIXED (heuristic improved; still not a real LLM — see below)
- **Evidence:** User supplied a real feedback PDF (unrelated product, Vietnamese-language, mixed formatting: lone `=>` lines whose instruction is on the *next* line, `=> text` inline arrows, `*BONUS:` asterisk-headers with no space after the marker, and Vietnamese imperative verbs like "Cần", "Đổi", "Thêm" instead of English trigger verbs or ASCII bullets). `AIService.extract_tasks_from_text` (`apps/api/src/services/ai.py`) is a line-anchored, English-only regex heuristic (explicitly documented as a placeholder for a real LLM call) — every line in that document would have matched zero patterns and the endpoint would have silently returned the single generic fallback task ("Review and implement ... specification"), for a document that actually contained 18 distinct, clearly actionable items.
- **Fix:** loosened the bullet pattern to not require a space after the marker (catches `*BONUS:`); added an arrow pattern for `=> text` inline and a standalone-`=>`-line lookahead that promotes the next line verbatim; added a small set of common Vietnamese task verbs (Thêm, Đổi, Cần, Tạo, Sửa, Nâng cấp, Cải thiện, Chỉnh, Xóa, Bỏ) plus the slang shorthand Up/Des seen in the doc ("Up lại ảnh...", "Des lại cho đẹp hơn").
- **Files:** `apps/api/src/services/ai.py`, `apps/api/tests/test_ai_service.py` (new `test_extract_tasks_from_vietnamese_feedback_doc`, built from real lines in the supplied document).
- **Known remaining limitation (by design, user's explicit choice):** this is still a regex heuristic, not a real LLM call — short unbulleted phrases with no trigger verb (e.g. "Error log Verbal") and mid-sentence lead-ins ("Để thêm tên vô") are still silently dropped. User was asked whether to wire up a real Claude API call instead and chose to keep the heuristic-only, no-new-dependency path; revisit if extraction quality on real documents keeps coming up short.
- **Lesson:** a placeholder explicitly documented as "replace with a real LLM" will keep failing in exactly the way its docstring warned about — every new real-world document is effectively a new heuristic-coverage bug waiting to be found, not a one-time fix.

### BUG-017 — BUG-016's fix still fell back to the generic task on the real PDF: PyPDF2 extracted one word per line
- **Status:** FIXED
- **Evidence:** User uploaded the real "Feedback web SAT.pdf" and got back only "Review and implement sat-champions specification" — BUG-016's Vietnamese-verb/arrow patterns still matched nothing. Rather than guess, queried the real, already-persisted `project_specifications.extracted_text` column directly from the live Supabase DB for that exact upload (`ProjectSpecification.extracted_text`, populated by `specs.py` before task extraction runs) and found the actual root cause: PyPDF2's `extract_text()` put **every single word on its own line** — `"Cần\n \ncó\n \ntính\n \nnăng\n \ntạo\n \nmục..."` instead of `"Cần có tính năng tạo mục..."`. Every line-anchored regex (all of BUG-016's new patterns included) saw one word per "line" and matched nothing — the docx-based tests never caught this because python-docx's paragraph extraction doesn't have this failure mode at all.
- **Fix:** added `_reflow_fragmented_pdf_text()` to `apps/api/src/services/document_parser.py`, applied to PyPDF2's output before it's stored/used. Empirically (from the real extracted text): exactly one blank line between two words is the fragmentation's word-gap artifact → merge with a space; two or more blank lines is a genuine line/paragraph break in the source PDF → keep as separate lines; zero blank lines between two already-multi-word tokens (i.e. extract_text() didn't fragment this particular file) → left untouched, unchanged from prior behavior. Verified directly against the real DB row: reflowing it turned 0 usable lines into 14, and `extract_tasks_from_text` went from the 1-item fallback to 8 real extracted tasks.
- **Files:** `apps/api/src/services/document_parser.py`, `apps/api/tests/test_document_parser.py` (new `test_reflow_recovers_lines_from_pypdf2_word_per_line_fragmentation`, built from a verbatim excerpt of the real failed upload's `extracted_text`).
- **Also tried and discarded:** a synthetic PDF built with one PDF `Tj` operator per word, meant to reproduce the fragmentation end-to-end through real PyPDF2 extraction rather than just unit-testing the reflow function. PyPDF2 actually concatenated those words with no separator at all (`"ImplementGitHubOAuthlogin"`) — a different failure mode than the real bug, not a faithful repro of whatever font/CMap property the user's real PDF has. Kept the test that operates on the real extracted text instead of shipping a misleading synthetic one.
- **Lesson:** `extracted_text` is persisted per spec upload — for any future "AI didn't extract anything from my real file" report, query the DB for that row first instead of trying to guess or reproduce the PDF locally. This is the second layer of the same underlying problem BUG-016 named: heuristics tuned against synthetic test strings will keep breaking against real extraction quirks (encoding, fragmentation, layout) that synthetic strings can't reproduce.
